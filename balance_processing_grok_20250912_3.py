...
class BalanceProcessor:
    ...
    p_new_valid_date => '{DEFAULT_NEW_VALID_DATE}'
                    ...
                    is_addition = False
                    if 'добавление статьи' in action_col.lower() or (pd.isna(action_col) and all(k in attrs for k in ['name', 'ord', 'lvl', 'parent'])):
                        is_addition = True
    ...
    p_article_id => {article_id}
            ...
            if generate_file:
                if comment_lines:
                    sql_queries.insert(5, "/*")  # После header
                    for line in comment_lines:
                        sql_queries.insert(6, line)
                    sql_queries.insert(6 + len(comment_lines), "*/")
                sql_queries.append("\n-- ===========================================")
...
def select_save_file(input_file: str):
    root = tk.Tk()
    root.withdraw()
    # Формируем имя выходного файла по умолчанию: [имя_входного_файла].sql
    default_filename = os.path.splitext(os.path.basename(input_file))[0] + ".sql"
...
def main():
    ...
    if execute_in_db:
        ...
        output_file = select_save_file(input_file)
        if not output_file:
